...
namespace KPZ_Restaurant_REST_API.Repositories
{
    public class ProductsRepository : RestaurantGeneric<Product>, IProductsRepository
    {
        ...
        {
            return await _context.Products.Where(p => p.RestaurantId == restaurantId && p.DeletedAt == null).Include(o => o.Category).ToListAsync();
        }

        public async Task<IEnumerable<Product>> GetAllProductsByCategoryName(int restaurantId, int categoryId)
        {
            return await _context.Products.Where(p => p.RestaurantId == restaurantId && p.CategoryId == categoryId && p.DeletedAt == null).Include(p => p.Category).ToListAsync();
        }
